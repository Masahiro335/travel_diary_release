...
return [
	...
	'Session' => [
		...
		'autoRegenerate'          => true, // 自動でセッションを再生成するのをやめる
		'checkAgent'              => false, // trueにすると毎回セッションが切れることがある
		'ini'                     => [
			'session.cookie_lifetime' => 15552000, //180days
			// TODO: パスを適切に決めないとセッションが切れる。一時的に無効化しておくが、再設定の必要があるか考えないといけない
			// 'session.cookie_path' => '/',
			// 'session.cookie_domain' => env('HTTP_HOST') ? env('HTTP_HOST') : 'localhost',
		],
